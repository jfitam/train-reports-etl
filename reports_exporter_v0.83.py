...
def get_error_line(e):
    ...
    line_number = traceback.extract_tb(e.__traceback__)[
        -1
    ].lineno  # Get the line number of the error
    return line_number
...
def prt_info(string, kind=logging.INFO, nl=True):
    time_log = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    # Get the current time
    info = f"[{time_log}] {string}"
...
try:
    ...
    prt_info(
        "There was a problem fetching the last version of the program. Skipping version check",
        logging.ERROR,
    )
...
try:
    if version > max_version and is_final_version:
        ...
        with alchemyEngine.connect() as conn:
            query = text(
                f"insert into \"AFC\".exporter_version_control(date, version) values ('{datetime.datetime.now()}','{version}')"
            )
            conn.execute(query)
            conn.commit()
        ...
        prt_info(
            "Current exporter is out of date. Please, use the last version to export data.",
            logging.ERROR,
        )
        sys.exit()
...
def get_report_name(excel_file_path, sheet=0):
    ...
    try:
        ...
        file_header = pd.read_excel(
            excel_file_path, sheet_name=sheet, nrows=50, header=None
        )
    except Exception as e:
        prt_info(
            f"There is a problem reading the file: {e}", kind=logging.ERROR
        )
        return 0, NO_REPORT
    ...
    for index, row in enumerate(file_header.values):
        ...
        # comparision
        if row.equals(train_list_header):
            return index + 1, TRAIN_LIST_REPORT
        elif row.equals(occupancy_header):
            return index + 1, OCCUPANCY_REPORT
        elif row.equals(bpd_header):
            return index + 1, BOOKING_PAYMENT_REPORT
...
def read_train_list(file_name, sheet=0):  # alchemyEngine,
    ...
    if name_report != TRAIN_LIST_REPORT:
        raise Exception(
            f"Wrong function invoked for sheet '{sheet}' from '{file_name}'"
        )
    ...
    try:
        ...
        for col in date_cols:
            df_file[col] = pd.to_datetime(
                df_file[col], errors="coerce", format="%Y-%m-%d"
            )
        ...
        for col in datetime_cols:
            df_file[col] = pd.to_datetime(
                df_file[col], errors="coerce", format="%Y-%m-%d %H:%M:%S"
            )
    ...
    try:
        ...
        if df_nan is not None:
            prt_info(
                f"{df_nan.shape[0]} registers from Train List have been removed because they were incorrect entries."
            )
            if not os.path.exists(export_folder):
                os.makedirs(export_folder)
            df_nan.to_csv(
                f"{export_folder}/Train List error rows {current_time}.csv.zip"
            )
    ...
    try:
        train_hours = pd.read_sql_table(
            "train_departure_times",
            alchemyEngine,
            schema="AFC",
            parse_dates={"departure_time": "%H:%M:%S"},
        )

    except Exception as e:
        raise Exception(
            f"Error fetching the departure times from database: {e}"
        )
    ...
    try:
        ...
        if df_file["Train Hour"].isnull().sum() > 0:
            trains_missing = df_file["Train Number"][
                df_file["Train Hour"].isnull()
            ].unique()
            raise Exception(
                f"There are missing departing hours in the database. Please, check the following trains: {', '.join(trains_missing)}"
            )

        # create extra columns
        df_file["Train Hour"] = df_file["Train Hour"].dt.strftime("%H:%M")
        df_file["Departure_Date_Short"] = df_file[
            "Departure Date"
        ].dt.strftime("%Y-%m-%d")
        df_file["Train-OD Short"] = (
            df_file["Train Number"] + " - " + df_file["OD"]
        )
        ...
        df_file["train_key"] = (
            df_file["Departure_Date_Short"] + " - " + df_file["Train-OD Short"]
        )

        # calculate the departing time of the train
        df_file["train_departure_date_time"] = pd.to_datetime(
            df_file["Departure_Date_Short"].astype(str)
            + " "
            + df_file["Train Hour"].astype(str)
        )
        train_date_adjustment = (
            df_file["train_departure_date_time"].dt.time
            > df_file["Departure Date"].dt.time
        )
        df_file["train_departure_date_time"] = df_file[
            "train_departure_date_time"
        ] - pd.to_timedelta(train_date_adjustment.astype(int), unit="D")
        df_file["train_departure_date_short"] = df_file[
            "Departure Date"
        ].dt.date - pd.to_timedelta(
            train_date_adjustment.astype(int), unit="D"
        )

        # calculate the services date (reduce one day if it is an early train before maintenance window)
        service_date_adjustment = df_file[
            "train_departure_date_time"
        ].dt.time <= datetime.time(5, 0)
        df_file["Service_Date"] = df_file[
            "train_departure_date_short"
        ] - pd.to_timedelta(service_date_adjustment.astype(int), unit="D")
    ...
    try:
        ...
        ticket_numbers = ", ".join(
            f"'{ticket}'" for ticket in df_file["Ticket Number"].unique()
        )
        query = f"""
        ...
        df_file = pd.merge(
            df_file, df_operation_date_times, on="Ticket Number", how="left"
        )
        df_file["operation_date"] = pd.to_datetime(
            df_file["operation_date_time"],
            errors="coerce",
            format="%Y-%m-%d %H:%M:%S",
        ).dt.strftime("%Y-%m-%d")
    ...
    try:
        ...
        def clean_phone(row):
            ...
            cleaned = re.sub(
                rf'^{re.escape(row["Prefix"])}', "", row["Telephone"]
            )
            # Remove dashes
...
def read_booking_payment(file_name, sheet=0):
    ...
    # check wrong lines. delete all nan or nat and save them in a separate file
    df_nan = df_file[
        df_file[df_file.columns.difference(nullable_cols)].isna().any(axis=1)
    ]
    df_file.dropna(
        inplace=True,
        how="any",
        subset=df_file.columns.difference(nullable_cols),
    )
    if not df_nan.empty:
        prt_info(
            f"{df_nan.shape[0]} registers from Booking Payment Detailed have been removed because they were incorrect entries."
        )
        if not os.path.exists(export_folder):
            os.makedirs(export_folder)
        df_nan.to_csv(
            f"{export_folder}/Booking Payment Detailed error rows {current_time}.csv.zip"
        )
    ...
    # clean the empty cells
    df_file.replace("", None, inplace=True)
    df_file.replace(" ", None, inplace=True)

    # drop vat penalty
    df_file["Penalty Tariff"] = df_file["Penalty Tariff"] * 1.15
    df_file.drop(labels="VAT Penalty", axis=1, inplace=True)
...
def read_occupancy(file_name, sheet=0):
    ...
    if not df_nan.empty:
        prt_info(
            f"{df_nan.shape[0]} registers from Occupancy have been removed because they were incorrect entries."
        )
        if not os.path.exists(export_folder):
            os.makedirs(export_folder)
        df_nan.to_csv(
            f"{export_folder}/Occupancy error rows {current_time}.csv.zip"
        )
    ...
    date_cols = [
        "Date"
    ]  # date is read as datetime but need to be parsed as date to make sure that duplicates are corretly found
    for col in date_cols:
        df_file[col] = df_file[col].dt.strftime("%Y-%m-%d")
    ...
    df_file["train_key"] = (
        pd.to_datetime(df_file["Date"]).dt.strftime("%Y-%m-%d")
        + " - "
        + df_file["Train Number"]
        + " - "
        + df_file["OD"]
    )
...
def get_date_pairs(df, column):
    try:
        ...
        raise Exception(
            f"There was an error while reading the dates of the report: {e}",
            logging.ERROR,
        )
...
def export_train_list(df_file, db_schema):
    ...
    with alchemyEngine.connect().execution_options(autocommit=True) as conn:
        ...
        if len(date_pairs) > 1:
            prt_info(
                "The dates on the report Train List are not consecutive. Make sure all the files of the day has been submitted",
        ...
        for date_from, date_to in date_pairs:
            try:
                delete_query = text(
                    f"DELETE FROM \"{db_schema}\".{table_name} WHERE departure_date_short between '{date_from}' and '{date_to}'"
                )
                conn.execute(delete_query)
                conn.commit()
                prt_info(
                    f"Previous data from {date_from} to {date_to} deleted successfully."
                )
            except Exception as e:
                conn.rollback()
                prt_info(
                    f"An error occurred while deleting the previous Train List data from {date_from} to {date_to} {e}",
                    kind=logging.ERROR,
                )
        ...
        for date, group in df_file.groupby("departure_date_short"):
            ...
            try:
                ...
                prt_info(
                    f"Data for {date} inserted successfully ({group.shape[0]} inserted)."
                )
            except Exception as e:
                conn.rollback()
                raise Exception(
                    f"An error occurred while inserting Train List data for {date}: {e}"
                )

            # register the audit table
            try:
                audit_query = text(
                    f"INSERT INTO \"AFC\".audit(timestamp, \"table\", operation, period, \"user\") VALUES ('{datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}', '{table_name}', 'insert', '{date}', '{os.getlogin()}')"
                )
                ...
                conn.rollback()
                raise Exception(
                    f"An error occurred while writing the audit information for Train List: {e}"
                )
...
def export_booking_payment(df_file, db_schema):
    ...
    dates = pd.to_datetime(
        df_file["operation_date_time"], format="%Y-%m-%d %H:%M"
    ).dt.date
    ...
    with alchemyEngine.connect().execution_options(autocommit=True) as conn:
        ...
        for date in unique_dates:
            ...
            try:
                delete_query = text(
                    f"DELETE FROM \"{db_schema}\".{table_name} WHERE to_char(operation_date_time, 'yyyy-mm-dd') = '{date}'"
                )
                conn.execute(delete_query)
                conn.commit()
                ...
                conn.rollback()
                prt_info(
                    f"An error occurred while deleting the previous Booking Payment Detailed data for date {date}: {e}",
                    kind=logging.ERROR,
                )
            ...
            try:
                ...
                conn.rollback()
                raise Exception(
                    f"An error occurred while inserting Booking Payment Detailed data for {date}: {e}"
                )
            ...
            try:
                ...
                conn.rollback()
                raise Exception(
                    f"An error occurred while writing the audit information for Booking Payment Detailed: {e}"
                )
...
def export_occupancy(df_file, db_schema):
    ...
    with alchemyEngine.connect().execution_options(autocommit=True) as conn:
        ...
        if len(date_pairs) > 1:
            prt_info(
                "The dates on the report Occupancy are not consecutive. Make sure all the files of the day has been submitted",
                kind=logging.WARNING,
            )
        ...
        for date_from, date_to in date_pairs:
            try:
                delete_query = text(
                    f"DELETE FROM \"{db_schema}\".{table_name} WHERE to_char(date, 'yyyy-mm-dd') between '{date_from}' and '{date_to}'and data_date = '{today}'"
                )
                conn.execute(delete_query)
                conn.commit()
                prt_info(
                ...
                conn.rollback()
                prt_info(
                    f"An error occurred while deleting the previous Occupancy data from {date_from} to {date_to}: {e}",
                    kind=logging.ERROR,
                )
        ...
        for date in unique_dates:
            ...
            try:
                ...
                conn.rollback()
                raise Exception(
                    f"An error occurred while inserting Occupancy data for {date}: {e}"
                )
            ...
            try:
                ...
                conn.rollback()
                raise Exception(
                    f"An error occurred while writing the audit information for Occupancy: {e}"
                )
    ...
    with alchemyEngine.connect().execution_options(autocommit=True) as conn:
        try:
            constraints_query = text(
                f"SELECT \"{db_schema}\".{remove_constraints_function} ('{db_schema}', '{table_name}')"
            )
            ...
            conn.rollback()
            prt_info(
                f"An error occurred while removing the constraints of the table {table_name}: {e}",
                kind=logging.ERROR,
            )
...
def add_constraints(db_schema, add_constraints_function):
    ...
    with alchemyEngine.connect().execution_options(autocommit=True) as conn:
        try:
            ...
            constraints_query = text(
                f'SELECT "{db_schema}".{add_constraints_function} ()'
            )
            conn.execute(constraints_query)
            conn.commit()
            ...
            conn.rollback()
            prt_info(
                f"An error occurred while adding the constraints {add_constraints_function}: {e}",
                kind=logging.ERROR,
            )
...
def read_file(report, file, sheet_name):
    try:
        ...
        if report == "Train List":
            df_file = read_train_list(file, sheet=sheet_name)
        elif report == "Booking Payment Detailed":
            df_file = read_booking_payment(file, sheet=sheet_name)
        elif report == "Occupancy":
            df_file = read_occupancy(file, sheet=sheet_name)
        else:
            prt_info(
                f"Reading of files {report} have not been implemented yet.",
                logging.WARNING,
            )
        ...
        prt_info(
            f"Error reading the sheet {sheet_name} from file {file}:{e}.",
            logging.ERROR,
        )
        return pd.DataFrame()  # return empty dataframe
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            if kind_file != NO_REPORT:
                ...
                if kind_file == TRAIN_LIST_REPORT:
                    kind_file_name = "Train List"
                elif kind_file == BOOKING_PAYMENT_REPORT:
                    kind_file_name = "Booking Payment Detailed"
                elif kind_file == OCCUPANCY_REPORT:
                    kind_file_name = "Occupancy"
                else:
                    kind_file_name = "Unknown"
                prt_info(
                    f"Found sheet '{sheet_name}' from '{file}' as {kind_file_name}"
                )
                ...
                if [file, sheet_name] not in files_found[kind_file_name]:
                    files_found[kind_file_name].append(
                        [kind_file_name, file, sheet_name]
                    )

            else:
                prt_info(
                    f"No report found in sheet '{sheet_name}' from '{file}'",
                    kind=logging.WARNING,
                )
...
for report in files_found:
    ...
    for report_name, file, sheet_name in files_found[report][:]:
        ...
        try:
            ...
            prt_info(
                f"Error on line {get_error_line(e)}: Reading of sheet '{sheet_name}' from '{file}' failed: {e}",
                logging.ERROR,
            )
    ...
    if df.empty:
        ...
        # order the dataframe
        if report == "Train List":
            sort_by = ["departure_date", "operation_date_time"]
        elif report == "Booking Payment Detailed":
            sort_by = ["operation_date_time"]
        elif report == "Occupancy":
            sort_by = ["ticket_reserved", "quota_configuration"]
        else:
            sort_by = []
        ...
        # remove duplicates
        if report == "Train List":
            subset_col = ["ticket_number"]
        elif report == "Booking Payment Detailed":
            subset_col = None
        elif report == "Occupancy":
            subset_col = ["date", "od", "train_number", "class"]
        else:
            subset_col = []
        ...
        if subset_col is not None:
            ...
            if duplicates.sum() > 0:
                ...
                df[duplicates].to_csv(
                    f"{export_folder}/{report} duplicates {current_time}.csv.zip"
                )
                df.drop_duplicates(
                    subset=subset_col,
                    keep="last",
        ...
        if not os.path.exists(export_folder):
            os.makedirs(export_folder)
        df.to_csv(
            f"{export_folder}/{report} data exported {current_time}.csv.zip",
            index=False,
            encoding="utf-8",
        )
        ...
        try:
            if report == "Train List":
                if is_above_threshold:
                    remove_constraints(db_schema, tbl_train_list)
                export_train_list(df, db_schema)
                if is_above_threshold:
                    add_constraints("AFC", "recreate_tl_constraints")
                prt_info(f"Report {report} exported successfully.")
            elif report == "Booking Payment Detailed":
                if is_above_threshold:
                    remove_constraints(db_schema, tbl_bpd)
                export_booking_payment(df, db_schema)
                if is_above_threshold:
                    add_constraints("AFC", "recreate_bpd_constraints")
                prt_info(f"Report {report} exported successfully.")
            elif report == "Occupancy":
                ...
                prt_info(
                    f"Exportation of report {report} have not been implemented yet.",
                    kind=logging.WARNING,
                )
        except Exception as e:
            prt_info(
                f"Error on line {get_error_line(e)} while exporting {report}: {e}",
                kind=logging.ERROR,
            )
            prt_info(
                f"Exportation of {report} failed. Exportation of the report aborted.",
                kind=logging.ERROR,
            )
...
if errors['errors_found']:
    messagebox.showinfo(
        "Alert",
        "There were ERRORS or WARNINGS during the exportation process. Please check file error_"
        + log_name
        + " for details.",
    )
else:
    ...
    messagebox.showinfo(
        "Alert", "The exportation process has been completed SUCCESSFUL."
    )
