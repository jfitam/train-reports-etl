...
general_handler = logging.FileHandler(log_name, mode='w')
general_handler.setLevel(logging.INFO)
error_handler = logging.FileHandler("error_" + log_name, mode='w')
...
def prt_info(string, kind=logging.INFO, nl=True):
    ...
    log.log(logging.DEBUG, f"The variable kind value is %s", errors_found)
...
for file in os.listdir("."):
    if file.endswith(".xlsx"):
        ...
        else: prt_info(f"Found file {file} but could not detect any report in it", kind=logging.WARNING)
...
for report in files_found:
    ...
    for file in files_found[report]:
        ...
        try:
            ...
            prt_info(e)
            prt_info(f"Reading of file {file} failed.", logging.ERROR)
    ...
    if df.empty:
        ...
        try:
            if(report == 'Train List'):
                ...
                prt_info(f"Exportation of report {report} have not been implemented yet.", kind='warning')
        except Exception as e:
            prt_info(e)
            prt_info("Exportation failed. Exportation of the report aborted.", kind='error')
            continue
...
log.log(logging.DEBUG, f"The variable errors_found value is %s", errors_found)
